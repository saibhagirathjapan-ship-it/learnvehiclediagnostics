...
title:
  en: "The ECU decides what it will serve"
  jp: "ECUは、何を実行するかを自分で決める"
short: { en: "How it decides", jp: "どう決めるか" }
...
:::en
If every check passes, the ECU serves the request and sends the positive reply. If any check fails, it **refuses** — and a refusal is exactly the **negative response** from H2: `7F`, the requested SID, and one reason code. The "no" is not a glitch on the wire. It is a decision the ECU makes inside itself.
:::
:::jp
すべての確認が通れば、ECUは要求を実行し、肯定応答を送ります。1つでも外れれば**断ります** ― その拒否こそ、H2で見た**否定応答**です：`7F`、要求されたSID、そして理由コード1つ。「いいえ」は回線上の不具合ではありません。ECUが自分の中で下す決定です。
:::

:::en
So the menu from the last card is not served flat. What an ECU will do right now depends on where it is: which session it holds, and whether it is unlocked.
:::
:::jp
つまり、前のカードのメニューは一律に出されるわけではありません。ECUが今なにを実行するかは、どこにいるか ― どのセッションを保ち、ロックが解けているか ― で決まります。
:::
