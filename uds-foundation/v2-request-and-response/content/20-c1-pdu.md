...
And that first byte? For a reply that worked, it is the **service identifier** — you meet it as `50` in the very next card.
jp: 最後に、全体が**長さ**を運びます ― 全部で6バイト。これで流れに継ぎ目ができました。

ヘッダ・パラメータ・長さ、この3つで、自分を説明できる1つの完全なメッセージ ― **A_PDU** ― になります。

そしてその先頭バイトは？　うまくいった応答では、それが**サービス識別子**です ― 次のカードで `50` として出会います。
