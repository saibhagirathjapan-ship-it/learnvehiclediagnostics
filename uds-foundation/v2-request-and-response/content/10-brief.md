...
2. **`+0x40` makes it positive** — the answer's first byte is just the request's first byte with one bit set.
...
2. **`+0x40` が「肯定」にする** ― 答えの先頭バイトは、リクエストの先頭バイトに1ビット立てただけ。
...
en: You can lay out the parts of a reply (content, control header, length), decode `+0x40` to turn a request SID into its positive-response SID (`10`→`50`, `22`→`62`), and read a sub-function echoed back in the answer.
jp: 応答の各部（中身・制御ヘッダ・長さ）を並べて示せ、`+0x40` を解いてリクエストSIDを肯定応答SIDに変換でき（`10`→`50`、`22`→`62`）、答えに反射されて返るサブファンクションを読めます。
:::
